...
# load the data
nonprob <- readRDS("PEW nonprob samples.RDS")
popdata <- readRDS("PEW population data.RDS")
...
religion.dist <- as.data.frame(table(popdata$RELIGCAT))
colnames(religion.dist) <- c("RELIGCAT", "Freq")
...
rakeddesign  <- rake(design=svy.unweighted,
                     sample.margins=list(~GENDER,~OWNGUN_GSS,~RELIGCAT,~FAMINC5,
                                         ~FOLGOV,~EDUCCAT5,~IDEO3), 
                     population.margins=list(gender.dist,gun.dist,religion.dist,
                                             inc.dist,folgov.dist,educ.dist,
                                             ideo.dist))
nonprob$rakeweights <- rakeddesign$prob

# get the projected voting distribution
svyby(~VOTESUM,~vendor, design=rakeddesign,svymean)[2:3]
Jakob <- svyby(~VOTESUM,~vendor, design=rakeddesign,svymean)
Jakob$name <- "Jakob"
```
...
rakeddesign  <- rake(design=svy.unweighted,sample.margins=list(~VOTE14,
                                                               ~GENDER,
                                                               ~EDUCCAT5,
                                                               ~RELIGCAT),
                     population.margins=list(vote.dist, gender.dist,
                                             edu.dist, reg.dist))

svyby(~VOTESUM,~vendor, design=rakeddesign,svymean)[2]

Gaja <- svyby(~VOTESUM,~vendor, design=rakeddesign,svymean)
Gaja$name <- "Gaja"
```
...
total <- rbind(Jakob[,c(2,3,8)],Gaja[,c(2,3,8)],Peter[,c(2,3,8)],raw[,c(2,3,8)])
...
print(result)
# Jakob 'wins' stratifying on 7 variables. My solutions gets a smaller MSE by weighting on an even larger set.
