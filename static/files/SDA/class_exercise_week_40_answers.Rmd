...
c) add the variable '$fpc' to the new sample, and specificy thesvydesign object
...
Now, you can either optimize your design based on costs or precision, or balance both. 
Imagine it costs 10 euros to conduct one interview (variable costs).

However, it also costs 1000 euros to conduct interviews in one cluster (fixed costs per cluster.

An SRS of size 200 will in this case normally cost you "100 *  10 + 5 * 1000 = 6000 euros. 

The cluster design specified in question 3 will cost you "100 * 10 + 2 * 1000 = 3000 euros. 

You now have to balance costs and precision. Assume that you have a fixed budget of 3000 euros. Doing an SRS is here not an option, because you would all your money on fixed costs. At the other extreme, you can sample in just 1 cluster and interview 300 cases. Can you work out, using the variable wgt (weight) as your dependent variable, what would be the right number of clusters to draw? We will disucss yoru answers in class at the start of the class in week 41.

(you may think that you would ideally like to stratify perhaps as well, either at the cluster-level or within the cluster. Indeed! That is what we will turn to next week through, for now, lets stick to clusters alone)
