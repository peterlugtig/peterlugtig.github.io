...
The ESS is a cross-national survey with data collection being conducted face-to-face. The sampling design varies slightly per country but in every country PSUs are drawn, and in every PSU a subset of individuals is selected. Some countries use stratification, either to select the PSUs, or select cases within PSUs. Lets load the data:
...
Specify the survey design object for Slovakia, using both the "PSU" variable to indicate clustering, and the "dweight"  variable as the weight variable. Ignore nonresponse for now. Please note that the "dweight"  variable adds up to the sample size. In earlier practicals, we always included an fpc-correction. If you want to do this correctly, multiply the dweight by 4261000/samplesize(SK) = {4261000/nrow(SK)} (adult population size Slovakia = 4.3 million).

```{r svydesign dweight, include=F}
SK$dweight2 <- SK$dweight*2295
SKdesign <- svydesign(ids=~PSU,weight=~dweight2,data=SK)
SKdesign
```
...
The ESS also includes Nonresponse weights, that correct for nonresponse using poststratification to the three-way crosstable of gender, age and educational level, as well as the sampling design. Can you plot the relation between "pspwght", "gndr", "agea"  and "eduyrs"? What is a strong predictor of nonresponse in the ESS-Slovakia? 
...
SKdesignNR <- svydesign(ids=~PSU,weight=~pspwght,data=SK)
...
Next week, we will talk in quite a bit of detail about how to design weights. Perhaps you noticed in question 6 that the standard error of our estimate becomes quite a bit larger when we weight for sampling and nonresponse. The great thing about weighting is that it can be used to correct for bias. This comes at the price of an increased variance. Can you investigate what the difference is between the highest and lowest pspwgt in the dataset? 
...
Some people (e.g. economists) argue that survey weighting is perhaps useful when you are estimating a point estimate like the proportion of daily Internet Users, but not so much when you are acually interested in a regression model.
...
1. Run the model first using unweighted data,   
2. a second time with just the design weights (use the svyglm function), and then   
...
# ad predicted values for "dailyint" to a new dataset
...
# generally, we find there is some NR correction, but not as strongly.
# why not?
...
## Question 10: population size equivalence weights
...
The variable "pweight" is a population size equivalence weight: it weights data from the different subpopulations when you combine them in one analysis. Using the variable "pweight", can you estimate the population size in Hungary, knowing that Slovakia's population is about 4.2 million?
...
## Question 12: 
Please note that the pweight is not centered at 1, and so we again do not take into account the fpc-correction. However, as the population is so large, this is not a big problem. Estimate the proportion of daily internet users in both countries. Use the svyby() command to do this (and look up how this works).
